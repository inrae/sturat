...
class TracegpxException extends Exception
{
}
/**
...
class Tracegpx extends PpciModel
{
  ...
  public $traittracefile = "modules/classes/traittrace.class.php";
  /**
   * Constructor
   *
   * @param PDO $bdd
  ...
  function importFileTrace(string $localfilename, string $filename, string $server, string $dbname, string $user, string $password): void
  {
    if (!is_file($localfilename)) {
      throw new TracegpxException(_("Le fichier à importer n'a pas été correctement téléchargé dans le serveur"));
    }
  ...
  function setTrace(int $trait_id, int $ogc_fid)
  {
    ...
    if (empty($trace["geom"])) {
      throw new TracegpxException(sprintf(_("La trace sélectionnée est vide pour le trait %s"), $trait_id));
    }
    if (!isset($this->traitTrace)) {
      $this->traitTrace = $this->classInstanciate("Traittrace", $this->traittracefile, true);
    }
